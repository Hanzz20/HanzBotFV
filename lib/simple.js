  ...
  let sock = Object.defineProperties(conn, {
    ...
    generateProfilePicture: {
      async value(buf) {
        ...
        let res = img.getWidth() > img.getHeight() ? img.resize(550, Jimp.AUTO) : img.resize(Jimp.AUTO, 650);
        //let img2 = await Jimp.read(await res.getBufferAsync(Jimp.MIME_JPEG));
        return {
          img: await res.getBufferAsync(Jimp.MIME_JPEG)
        };
      },
      writable: false,
      enumerable: true
    ...
    updateProfilePict: {
      async value(jid, content) {
        const {
          img
        } = await conn.generateProfilePicture(content)
        await conn.query({
          tag: "iq",
          attrs: {
    ...
    sendContact: {
      ...
      async value(jid, data, quoted, options) {
        ...
        let contacts = []
        for (let [number, name, email, website] of data) {
...
item2.X-ABLabel:Email` : ""}${website ? `
item${email ? 3 : 2}.URL:${website}
item${email ? 3 : 2}.X-ABLabel:Website` : ""}
...
END:VCARD
    ...
    reply: {
      ...
      value(jid, text = "", quoted, options) {
        return Buffer.isBuffer(text) ? conn.sendFile(jid, text, "file", "", quoted, false, {
    ...
    sendExternalAdReply: {
       ...
       * @param {String} url
       * @param {import("@adiwajshing/baileys").proto.WebMessageInfo} quoted
       * @result {import("@adiwajshing/baileys").proto.WebMessageInfo}
       */
      async value(jid, text = "Hello World", title = "Ini title", body = "Ini body", thumbnail = null, thumbnailUrl = "", url = "https://example.com/", quoted) {
    ...
    sendButtonLocation: {
       ...
       * @param {Object} options
       */
      value: async (jid, buffer, content, footer, button1, row1, quoted, options = {}) => {
    ...
    sendListMessage: {
      async value(jid, text = "This is text", footer = "This is the footer", title = "This is the title", buttonText = "Click me!", content = [
        ["Section 1", [{
          title: "1",
          rowId: "1",
          description: "First"
        ...
        for (let i of content) {
          if (!i[0] || !Array.isArray(i[1])) continue
          let obj = {}
          obj.title = i[0]
          obj.rows = i[1]
          sections.push(obj)
        ...
        if(!quoted.message && quoted.quoted) quoted = quoted.quoted
    ...
    fReply: {
       ...
       * @param {String} jid
       * @param {String} text
       * @param {Buffer|String<url>} buffer
       * @param {String} fakeJid
       * @param {String} fakeText
       * @param {String} fakeGroupJid
       * @param {Buffer|String<url>} fakeBuffer
       * @param {String} options
       */
      async value(jid, text = "Hello", buffer, fakeJid = this.user.jid, fakeText = "Hello", fakeGroupJid, fakeBuffer, options) {
        if (typeof fakeBuffer == "string" && (fakeBuffer.startsWith("https://") || fakeBuffer.startsWith("http://"))) fakeBuffer = await (await fetch(fakeBuffer)).buffer()
        if (typeof buffer == "string" && (buffer.startsWith("https://") || buffer.startsWith("http://"))) buffer = await (await fetch(buffer)).buffer()
        if (!(fakeBuffer instanceof Buffer)) fakeBuffer = null
        if (!(buffer instanceof Buffer)) buffer = null

        let img
        if (fakeBuffer) img = await extractImageThumb(fakeBuffer, 300)
        ...
        return await conn.sendMessage(jid, msg, {
          quoted: {
            key: {
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? {
          ...
          text: {
            get() {
              return text || this.caption || this.contentText || this.selectedDisplayText || this?.documentWithCaptionMessage?.message?.documentMessage?.caption || ""
            },
            enumerable: true
          },
          mentionedJid: {
          ...
          reply: {
            ...
            value(text, chatId, options) {
              return self.conn?.reply(chatId ? chatId : this.chat, util.format(text), this.vM, options)
            },
            enumerable: true,
          },
          copy: {
    ...
    text: {
      get() {
        const msg = this.msg
        const text = (typeof msg === "string" ? msg : msg?.text) || msg?.caption || msg?.contentText || msg?.message?.documentMessage?.caption || ""
    ...
    reply: {
      value(text, chatId, options) {
        return this.conn?.reply(chatId ? chatId : this.chat, util.format(text), this, options)
      }
    },
    copy: {
      value() {
