...
const clockString = (ms) => {
  ...
  return d.toString().padStart(2, "0") + ", " + [h, m, s].map(v => v.toString().padStart(2, 0)).join(":")
}
...
const random = function(min = 1, max = 100) {
...
async function ss(url, options = {}) {
  ...
  if(options.__proto__ != Object.prototype) options = {}
  ...
  try {
    if(typeof options["User-Agent"] == "string") await page.setUserAgent(options["User-Agent"])
    ...
    if(options.wait || options.sleep) await page.waitForTimeout(options.wait || options.sleep)
    ...
    if(error && error.message.includes("ERR_NAME_NOT_RESOLVED")) return {
      error: true,
